...
const profiles = [
  ...
  },
  {
    ...
    name: "Rakib Hasan",
    age: 30,
    type: "Male", // added missing fields for consistency
    occupation: "",
    height: "",
    weight: "",
    dob: "",
    ...
    <div className="max-w-3xl mx-auto p-6">
      <div className="bg-white shadow-md rounded-lg flex p-6">
        <img
          src={profile.image}
          alt="Profile"
          className="w-40 h-48 rounded mx-4 object-cover"
        />
        <div>
          <h2 className="text-2xl font-bold mb-2">{profile.name}</h2>
          <div className="space-y-1">
            <div>Type: {profile.type || "N/A"}</div>
            <div>Age: {profile.age || "N/A"}</div>
            <div>Height: {profile.height || "N/A"}</div>
            <div>Weight: {profile.weight || "N/A"}</div>
            <div>Occupation: {profile.occupation || "N/A"}</div>
            <div>Race: {profile.race || "N/A"}</div>
            <div>DOB: {profile.dob || "N/A"}</div>
            <div>Division: {profile.division || "N/A"}</div>
          </div>
